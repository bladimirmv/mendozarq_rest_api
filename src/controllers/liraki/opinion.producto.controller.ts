...
export const addOpinionProducto = async (req: Request, res: Response) => {
  try {
    ...
    if (opiniones.length >= 2) {
      return res.status(400).json({
        message: `No se ha podido registrar, has llegado al limite de opiniones. 🙁`,
      });
    }
...
export const getAllOpinionProductoByUuid = async (req: Request, res: Response) => {
  try {
    ...
    const [opiniones]: [any[], FieldPacket[]] = await conn.query(
      `SELECT * FROM opinionProducto WHERE uuidProducto = ? ORDER BY creadoEn DESC`,
      [uuid]
    );

    const [usuarios]: [any[], FieldPacket[]] = await conn.query(
      `SELECT u.* FROM opinionProducto as op
		INNER JOIN usuario u on op.uuidCliente = u.uuid
		WHERE op.uuidProducto = ? ORDER BY op.creadoEn DESC`,
      [uuid]
    );
