...
export const addCarpetaProyecto = async (req: Request, res: Response) => {
  try {
    ...
    const [[existCarpeta]]: [any[], FieldPacket[]] = await conn.query(
      'SELECT * FROM carpetaProyecto WHERE  nombre = ? and uuidProyecto = ?',
      [carpeta.nombre, carpeta.uuidProyecto]
    );

    if (existCarpeta) {
      return res.status(400).json({
        message:
          'Ya existe una carpeta con el mismo nombre, por favor ingrese otro en su lugar. 🙁',
      });
    }
...
export const getOneCarpetaProyecto = async (req: Request, res: Response) => {
  try {
    ...
    const [[carpeta]]: [any[], FieldPacket[]] = await conn.query(
      'SELECT * FROM carpetaProyecto WHERE uuid = ?',
      [uuid]
    );
...
export const getAllCarpetaProyecto = async (req: Request, res: Response) => {
  try {
    ...
    const [carpetas]: [any[], FieldPacket[]] = await conn.query(
      'SELECT * FROM carpetaProyecto ORDER BY creadoEn DESC'
    );
...
export const getAllCarpetaProyectoByUuid = async (
  ...
  try {
    ...
    const [carpetas]: [any[], FieldPacket[]] = await conn.query(
      'SELECT * FROM carpetaProyecto WHERE uuidProyecto = ? ORDER BY creadoEn DESC',
      [uuid]
    );
...
export const updateCarpetaProyecto = async (req: Request, res: Response) => {
  try {
    ...
    const [[existNombre]]: [any[], FieldPacket[]] = await conn.query(
      'SELECT * FROM carpetaProyecto WHERE  nombre = ? AND uuid != ? AND uuidProyecto = ?',
      [carpeta.nombre, uuid, existCarpeta.uuidProyecto]
    );
    if (existNombre) {
      return res.status(404).json({
        message:
...
export const getAllDocumentoProyectoByUuid = async (
  ...
  try {
    ...
    const [documentos]: [any[], FieldPacket[]] = await conn.query(
      `SELECT * FROM documentoProyecto WHERE uuidProyecto = ? AND path = ? ORDER BY creadoEn DESC`,
      [uuid, path]
    );
...
export const deleteDocumento = async (req: Request, res: Response) => {
  try {
    ...
    const [[row]]: [any[], FieldPacket[]] = await conn.query(
      'SELECT * FROM documentoProyecto WHERE uuid = ? ',
      [uuid]
    );
...
export const updateDocumentoProyecto = async (req: Request, res: Response) => {
  try {
    ...
    const [[existDoc]]: [any[], FieldPacket[]] = await conn.query(
      'SELECT * FROM documentoProyecto WHERE uuid = ?',
      [uuid]
    );
    if (!existDoc) {
    ...
    const [[existNombre]]: [any[], FieldPacket[]] = await conn.query(
      'SELECT * FROM documentoProyecto WHERE  nombre = ? AND uuid != ? AND path = ? AND uuidProyecto = ?',
      [documento.nombre, uuid, existDoc.path, existDoc.uuidProyecto]
    );
    if (existNombre) {
      return res.status(404).json({
        message:
          'Ya existe un documento con el mismo nombre, por favor ingrese otro en su lugar. 🙁',
      });
    }
