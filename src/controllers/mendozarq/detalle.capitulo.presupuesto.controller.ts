...
export const getOneDetalleCapituloPresupuesto = async (req: Request, res: Response) => {
  try {
    ...
    const [[detalleCapitulo]]: [any[], FieldPacket[]] = await conn.query(
      `SELECT * FROM detalleCapitulo WHERE uuid = ?`,
      [uuid]
    );
...
export const getAllDetalleCapituloPresupuesto = async (req: Request, res: Response) => {
  try {
    const conn: Pool = await connect();

    const [detalleCapitulo]: [any[], FieldPacket[]] = await conn.query(
      `SELECT * FROM detalleCapitulo GROUP BY descripcion ORDER BY descripcion`
    );
...
// ====================>
export const getAllDetalleCapitulosByProyecto = async (req: Request, res: Response) => {
  try {
    const conn: Pool = await connect();
    const uuid: string = req.params.uuid;
    ...
    const [presupuestoobra]: [any[], FieldPacket[]] = await conn.query(
      `SELECT d.*
      FROM detalleCapitulo AS d
      LEFT JOIN capituloPresupuesto c on d.uuidCapituloPresupuesto = c.uuid
      LEFT JOIN presupuestoObra po on c.uuidPresupuestoObra = po.uuid
      LEFT JOIN proyecto p on po.uuidProyecto = p.uuid
      WHERE p.uuid = ? ORDER BY d.creadoEn;`,
      [uuid]
    );
...
export const updatedetalleCapituloPresupuesto = async (req: Request, res: Response) => {
  try {
    ...
    const [[presupuesto]]: [any[], FieldPacket[]] = await conn.query('SELECT * FROM detalleCapitulo WHERE uuid = ?', [
      uuid,
    ]);
